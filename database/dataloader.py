...
def Contrastive_Loss(batch_Teacher, batch_Student):
    ...
    B_t, Dim_t = np.array(batch_Teacher.shape)[
        0], np.array(batch_Teacher.shape)[1]
    B_s, Dim_s = np.array(batch_Student.shape)[
        0], np.array(batch_Student.shape)[1]

    # Création d'un tenseur 3D pour vectoriser l'opération de soustraction paire à paire
    T_rep = batch_Teacher.unsqueeze(1).expand(
        B_t, B_t, Dim_t)  # (B, 1, dim) -> (B, B, dim)
    S_rep = batch_Student.unsqueeze(0).expand(
        B_s, B_s, Dim_s)  # (1, B, dim) -> (B, B, dim)
...
class BatchMaker:
    ...
    def init_area_ref(self, patch_size):
        ...
        patch_size : tuple (h,w)
            Taille du patch : h := nombre de pixel du patch selon l'axe 0 (hauteur)
                            : w := nombre de pixel du patch selon l'axe 1 (largeur)
        '''
        Area_REF, _, _ = create_selection_area(
            self.h_img, self.w_img, patch_size[0], patch_size[1])
    ...
    def make_batch_M0(self, P, patch_size,
        ...
        if len(ind_list) < P:
            print(
                f"Le nombre de patchs fournis ({len(ind_list)}) sera inférieur à celui demandé ({P})")
            P = len(ind_list)
        ...
        for b in range(P):
            ...
            # la position choisie n'est plus disponible
            Area[ind_list[i]] = False
    ...
    def make_img_het(self, channel):
        ...
        self.img_het = np.stack(
            (self.Re[:, :, channel[0]], self.Re[:, :, channel[1]]))
